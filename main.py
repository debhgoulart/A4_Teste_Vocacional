...
def realizar_perguntas():
    ...
    for i, (pergunta, respostas) in enumerate(perguntas, start=1):
        print(f"\nPergunta {i}: {pergunta}")
        for idx, (resposta, _) in enumerate(respostas, start=1):
            print(f"{idx} - {resposta}")
        escolha = int(input("Escolha uma opção (1, 2 ou 3): "))
        pontuacao_total += respostas[escolha - 1][1]

    return pontuacao_total

def main():
    pontuacao = realizar_perguntas()
    prolog = Prolog()
    prolog.consult("carreiras.pl")
    query = list(prolog.query(f"encontrar_carreira({pontuacao}, Carreira)"))
    if query:
        carreira_sugerida = query[0]['Carreira']
        carreira_legivel = carreira_sugerida.replace("_", " ").capitalize()
        print(f"\nCom base em suas respostas, a carreira sugerida é: {carreira_legivel}")
        
    else:
        print("\nNenhuma carreira encontrada para sua pontuação.")
